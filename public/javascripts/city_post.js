$(document).ready(function() {
    ...
    $(".withoutimage").click(function() {
      if ($("#post").val() == "" || $("#post").val() == []) {
        ...
        $.post("/city_post/insertwithoutimage", insertobj, function data() {
...
timeline += `<i class="fa fa-thumbs-up like" id=${item.id} "  style="color:#33ccff;float:right;position:relative;right:45px;bottom:10px"></i>` 
             ...
             timeline +=`     <a href="/city_post/comment?id=${item.id} "  class=" btn-xs" style="float:right;position:relative;bottom:15px;" ><i class="fa fa-comment"></i></a>
                </p>
              </div>
            </li>`;
      });
    ...
    $("#result").on("click", ".like", function() {
      ...
      $.post('/city_post/insertlike',{city_postid,status},function data(){
        refresh()
      })
    })
...
$.post('/city_post/favourite',{city_postid,favourite},function data(){
  refresh()
})
