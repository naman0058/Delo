$(document).ready(function() {
    ...
    $(".withoutimage").click(function() {
      if ($("#post").val() == "" || $("#post").val() == []) {
        ...
        $.post("/lab_post/insertwithoutimage", insertobj, function data() {
      ...
      $.each(data, function(i, item) {
        ...
        timeline += `<li>
            ...
            <div class="tm-box">
             ...
             timeline +=`   <i class="fa fa-heart favourite" id= "${item.id}" style = "background-color:ff0066;float:right;position:relative;bottom:10px;"></i>`
                }else{
...
timeline += `<i class="fa fa-thumbs-up like" id=${item.id} "  style="color:#33ccff;float:right;position:relative;right:45px;bottom:10px"></i>` 
          ...
          <i class="fa fa-thumbs-up like" id=${item.id} "  style="float:right;position:relative;right:45px;bottom:10px"></i>`
               ...
               timeline +=`    <a href="/lab_post/comment?id=${item.id} "  class=" btn-xs" style="float:right;position:relative;bottom:15px;" ><i class="fa fa-comment"></i></a>
                </p>
              </div>
            </li>`;
      });
    ...
    $("#result").on("click", ".like", function() {
      const city_postid = $(this).attr("id");
      const status = "like";
      $(this).toggleClass("likes");
      $.post('/lab_post/insertlike',{city_postid,status},function data(){
        refresh()
      })
    })  
    ...
    $.post('/lab_post/favourite',{city_postid,favourite},function data(){
      refresh()
    })
